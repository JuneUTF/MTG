...
function checkDay() {
    if (compareDates()) {//正しい場合
        ...
        setDay();
        checkTimeTowDay();
        checkDayBoolean = true;
        checkBtn()
    } else {
        msg.textContent = '過去の日付を選択しないでください。';
        setDay();
        checkDayBoolean = false;
        checkBtn()
    }
}
// 目的情報を取得する非同期関数
...
time_start.addEventListener('input', EventTimeStart);

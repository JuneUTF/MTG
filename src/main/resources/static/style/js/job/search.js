...
//開始日付の初期値を現在の日付に設定
searchData.date_start == null ? date_start.value = new Date().toISOString().substring(0, 10) : date_start.value = searchData.date_start;
//終了日付の初期値を現在の日付に設定
searchData.date_end == null ? "" : date_end.value = searchData.date_end;
// 内容入力フィールドを取得
...
async function callPurpose() {
    try {
        ...
        data.map((e) => {
            ...
            if(e.id ==searchData.purposeId){
                purposeId =  `<option value="${e.id}">${e.purpose}</option>`;
            }else{
                purposeHTML = purposeHTML + `<option value="${e.id}">${e.purpose}</option>`;
            }
        });
        purposeHTML = `<select id="purpose" name="purposeId">${purposeId}<option value="0">全部</option>${purposeHTML}</select>`;
    } catch (error) {
        //APIを呼び出しできない場合 input box 表示されます。
...
async function callCharge() {
    try {
        ...
        data.map((e) => {
            //APIを呼び出し場合 選択ボックスを表示されます。
            if(e.id ==searchData.chargeId){
                chargeId = `<option value="${e.id}">${e.charge}</option>`;
            }else{
            chargeHTML = chargeHTML + `<option value="${e.id}">${e.charge}</option>`;
            }
        });
        chargeHTML = `<select id="charge" name="chargeId">${chargeId}<option value="0">全部</option>${chargeHTML}</select>`;
    } catch (error) {
        //APIを呼び出しできない場合 input box 表示されます。
        chargeHTML = `<input type="text" id="charge" name="chargeId" placeholder="内容を入力ください。" required>`;
    }
